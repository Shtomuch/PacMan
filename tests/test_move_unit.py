...
def test_move_unit_initial_direction(
        move_unit,
        new_direction,
        expected_direction):
...
def test_move_unit_wall_collision(move_unit):
    ...
    c = Coordinate.get_tile_center(
        move_unit.coordinates.x_tile, move_unit.coordinates.y_tile).y_global
    move_unit.move(0.1, 1)
    # Персонаж не повинен пройти крізь стіну
    assert abs(move_unit.coordinates.y_global - c) < 1e-10
