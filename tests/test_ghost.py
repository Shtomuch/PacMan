...
from classes.ghost import Ghost, Blinky, Pinky, Inky, Clyde
...
def setup_game():
    ...
    GlobalVars.pacman = type("Pacman", (), {
        "move_unit": type("MoveUnit", (), {"coordinates": Coordinate(100, 100), "direction": 0})()})
    ...
    GlobalVars.tilemap.house = Coordinate(150, 150)  # Конкретні координати будинку
    GlobalVars.tilemap.width = 28  # Розмір мапи, щоб уникнути помилки з delta
    GlobalVars.tilemap.height = 31

    GlobalVars.ghosts = [Blinky(Coordinate(50, 50)), Pinky(Coordinate(50, 50)), Inky(Coordinate(50, 50)),
                         Clyde(Coordinate(50, 50))]
...
@pytest.mark.parametrize("state, expected_speed", [("alive", 4.2), ("frightened", 3.5), ("dead", 9), ("in_house", 2)])
